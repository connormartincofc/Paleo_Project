...
# describes app and is where you put things like buttons and timesliders
  ui <- fluidPage(
    ...
    output$leafletPlot <- renderLeaflet({
      leaflet(subset(dat, date_BP > input$BP)) %>%
        ...
        addLegend(position = c("topleft"), pal = happal,
                  values = ~mtDNA_hap_simp, labels = "haplogroups",
                  title = "mtDNA haplogroups")})
    }
  }
